...
const BoardWrite = () => {
    ...
    const postData = () => {
        if (title === '') {
            ...
            axios.post(`${process.env.REACT_APP_EC2_API_URL}/boardWrite`, {title : title, location : location, userId : sessionStorage.getItem('userId'), content : content }, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
